...
$(document).ready(function () {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            trendingdata = fulldata['trending'];
            trending_list(-1);
            specialsdata = fulldata['specials'];
            filtereddata('get_specials_list', 'specialsdata');
            standup_videos = fulldata['standup'];
            filtereddata('standup_list', 'standup_videos');
            comicdata = fulldata['allcomics'];
            filtereddata('all_comics_cards');
            populardata = fulldata['popular'];
    ...
    xmlhttp.open("GET", "/server/database.php?q=data", true);
    xmlhttp.send();
});
